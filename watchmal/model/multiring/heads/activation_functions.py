"""
Normalized activations for the query-transformer head.

Trimmed from https://github.com/deep-spin/entmax to the transforms actually used
by the model: softmax, sparsemax and 1.5-entmax. (The arbitrary-alpha bisection
variant and the unused nn.Module wrapper were removed.)
"""
...
def apply_normalized_activation(
    X,
    activation="softmax",
    dim=-1,
    k=None,
    ...
    **_,
    ...
    if activation == "softmax":
        ...
        return sparsemax(X, dim=dim, k=k, return_support_size=return_support_size)
    elif activation == "entmax15":
        return entmax15(X, dim=dim, k=k, return_support_size=return_support_size)
    else:
        raise ValueError("activation must be one of: 'softmax', 'sparsemax', 'entmax15'")
