...
class NodeEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, dropout=0.0):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.LayerNorm(hidden_channels),    # already there, keep it
...
class CheRP(nn.Module):
    def __init__(
        ...
        self.num_special_tokens    = 1   # CLS only; hint (if used) is folded into its init, never touches nodes
    ...
    def forward(self, data):
        ...
        # Windowed tokens occupy the trailing num_tokens slots; CLS sits at the front
        # and is excluded from node edges. Indices are local to `nodes`/`tokens`
        # respectively rather than a combined array.
        ...
        for i in range(self.num_layers):
            ...
            if i < self.num_layers - 1:
                ...
                # (CLS excluded), so this is a dense gather, not an edge list
                node_kv_toks = tokens_seq[:, self.num_special_tokens:, :]
                nodes = self.t2n_layers[i](nodes, node_kv_toks, batch, log_stats=ls)

        token_out = tokens.view(batch_size, self.total_tokens, -1)

        # Token similarity — only computed when logging (includes CLS at slot 0)
        if ls:
            with torch.no_grad():
                token_norm = F.normalize(token_out, p=2, dim=-1)
                sim = torch.bmm(token_norm, token_norm.transpose(1, 2)).mean(dim=0)
                sim.fill_diagonal_(0)
                self.token_sim        = sim.sum(dim=-1) / (self.total_tokens - 1)
